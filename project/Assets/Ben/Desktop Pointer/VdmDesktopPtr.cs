...
public class VdmDesktopPtr : MonoBehaviour
{
    [HideInInspector]
    public int Screen = 0;
    [HideInInspector]
    ...
    private VdmDesktopManagerPtr m_manager;
    ...
    void Start()
    {
        m_manager = transform.parent.GetComponent<VdmDesktopManagerPtr>();
    ...
    public void CheckKeyboardAndMouse()
    {
        ...
        {
            VdmDesktopManagerPtr.ActionInThisFrame = true;
        ...
        if (Visible())
        {
            ...
            foreach (RaycastHit rcast in rcasts)
            {
                ...
                if (m_lastShowClick == 0)
                {
                    ...
                    if (m_manager.ViveZoom != VdmDesktopManagerPtr.ViveButton.None)
                    {
                ...
                if (m_manager.ViveLeftClick != VdmDesktopManagerPtr.ViveButton.None)
                {
    ...
    Valve.VR.EVRButtonId MyButtonToViveButton(VdmDesktopManagerPtr.ViveButton button)
    {
        if (button == VdmDesktopManagerPtr.ViveButton.Grip)
            return Valve.VR.EVRButtonId.k_EButton_Grip;
        else if (button == VdmDesktopManagerPtr.ViveButton.Trigger)
            return Valve.VR.EVRButtonId.k_EButton_SteamVR_Trigger;
        else if (button == VdmDesktopManagerPtr.ViveButton.Menu)
